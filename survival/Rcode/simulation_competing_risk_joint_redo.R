...
groups = list()
friends = list()
inidividual_data = list()
for(i in (n_person*n_each_person + 1):(n_person*n_each_person + n_person*event_num)){
    groups[[i]] = i
    if(i < n_person*n_each_person + n_person){
        friends[[i]] = c(i, i + n_person, i+ 2*n_person)
    }else if(i < n_person*n_each_person + 2*n_person){
        friends[[i]] = c(i - n_person, i ,i+ n_person)
    }else{
        friends[[i]] = c(i - 2*n_person, i - n_person, i)
    }
}

for(i in 1:n_person){
    inidividual_data[[i]] = friends[[i+n_person*n_each_person]]
}
...
inidividual_idx = 1
for(event_idx in 1:event_num){
    args[[event_idx]] = inla.likelihood.parser(res$misc$configs$config[[config_idx]]$arg.str[inidividual_data[[inidividual_idx]][event_idx]])
}
sample_i(1000,args)
#pi(time = t, event = j)
...
conditional_logarithmic_score = function(res,lower = 0){
    ...
    #get inidividual_data from somewhere
    #get Time from somewhere
    #get Event from somewhere
    #get mode config_idx
    config_idx = 1
    log_density = rep(NA,n_person)
    for(inidividual_idx in 1:n_person){
        if(Time[inidividual_idx] > lower & Event[inidividual_idx] != 0){
            for(event_idx in 1:event_num){
                args[[event_idx]] = inla.likelihood.parser(res$misc$configs$config[[config_idx]]$arg.str[inidividual_data[[inidividual_idx]][event_idx]])
                args[[event_idx]]$linear.predictor = mode$eta[inidividual_data[[inidividual_idx]][event_idx]]
            }
            log_density[inidividual_idx] = density_i(time = Time[inidividual_idx],event = Event[inidividual_idx],args = args,islog = TRUE) - surv_i(lower = lower,args = args,islog = TRUE)
        }
    }
    return(log_density)
}
...
conditional_prob = function(res,lower,upper){
    ...
    for(inidividual_idx in 1:n_person){
        ...
        prob[inidividual_idx,] = conditional_prob_i(lower = lower,upper = upper, args = args)
    }
    return(prob)
}
...
AUC_compute = function(res = res,lower,upper,prob = NULL){
    ...
    for(event_idx in 1:event_num){
        ...
        for(inidividual_idx in true_fold){
            AUC[event_idx] = AUC[event_idx] + sum(prob[inidividual_idx,event_idx] > prob[false_fold,event_idx])
        }
    ...
    for(inidividual_idx in true_fold){
        AUC[event_num+1] = AUC[event_num+1] + sum(prob[inidividual_idx,event_num+1] > prob[false_fold,event_num+1])
    }
