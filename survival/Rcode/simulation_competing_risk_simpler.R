...
y_surv1 = inla.surv(time = c(Time,rep(NA,n_person),rep(NA,n_person)),event = c(Event1,rep(NA,n_person),rep(NA,n_person)))
y_surv2 = inla.surv(time = c(rep(NA,n_person),Time,rep(NA,n_person)),event = c(rep(NA,n_person),Event2,rep(NA,n_person)))
y_surv3 = inla.surv(time = c(rep(NA,n_person),rep(NA,n_person),Time),event = c(rep(NA,n_person),rep(NA,n_person),Event3))

id_surv1 = c(1:n_person,rep(NA,n_person),rep(NA,n_person))
id_surv2 = c(rep(NA,n_person),1:n_person,rep(NA,n_person))
id_surv3 = c(rep(NA,n_person),rep(NA,n_person),1:n_person)

age_surv1 = c(age,rep(NA,n_person),rep(NA,n_person))
age_surv2 = c(rep(NA,n_person),age,rep(NA,n_person))
age_surv3 = c(rep(NA,n_person),rep(NA,n_person),age)

mu = as.factor(c(rep(1,n_person),rep(2,n_person),rep(3,n_person)))

data_list = list(y = list(y_surv1,y_surv2,y_surv3),
                 mu = mu,
                 id_surv1 = id_surv1,
                 id_surv2 = id_surv2,
                 id_surv3 = id_surv3,
...
hyper_fixed = FALSE
hyper_initial = c(theta,theta,theta)

formula = y ~ -1  + mu + age_surv1 + age_surv2 + age_surv3

res = inla(formula, 
           family = c("weibullsurv","weibullsurv","weibullsurv"),
           data = data_list, 
           control.family = list(list(variant = 1,initial = hyper_initial[1], fixed = hyper_fixed),list(variant = 1,initial = hyper_initial[2], fixed = hyper_fixed),list(variant = 1,initial = hyper_initial[3], fixed = hyper_fixed)),
...
for(individual.interest in 1:n_person){
    if(Time[individual.interest] > s){
        for(point.interest in individual_SV[[individual.interest]]){
            groups[[point.interest]] = individual_SV[[individual.interest]]
        }
    }
}
...
scores_computing = function(sample){
    ...
    for(individual.interest in 1:n_person){
        if(Time[individual.interest] >= s){
            brier_score[individual.interest] = 0
    ...
    return(list(S_AUC = S_AUC,brier_score = mean(brier_score),log_conditional_density = mean(log(conditional_density))))
}
...
eta_true = c(eta_event1,eta_event2,eta_event3)
